#Importing Libraries to be used in this program
...
knn_clf.fit(inputs.values,target.values.ravel())
...
validation_converted = np.array(validation, dtype=float)
count=0
total = len(error)
#in order to measure time , signalizing the starting point to note time
start2 = datetime.datetime.now()
for i in validation_converted:
    prediction = knn_clf.predict([i])
